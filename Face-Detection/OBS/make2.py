...
def main(embeddings_path):
    ...
    while True:
        ...
        if unknown_face_detecion_status:
            ...
            for location,i, distance in zip(locations, range(0,len(distances)), distances):
                ...
                if distance < 0.6:
                    ...
                    cv2.putText(image, person[i], (location[3], location[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
                    #cv2.putText(image, f"Person {i+1}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    break
                else :
                    
                    cv2.rectangle(image, (location[3], location[0]), (location[1], location[2]), (0, 255, 0), 2)
                    #cv2.putText(image, f"Person UNKNOWN", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    cv2.putText(image, person['unk'], (location[3], location[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
